...
**Effort**: `xhigh` by default; fable implementation may run `high`. `max` only after an xhigh attempt falls short. Verification runs at the same or higher model + effort tier than the work it checks; lanes the Models table routes to gpt-5.6-sol (refuter passes, security review/audit, verification of security-sensitive code) count as same-tier at `xhigh` — escalation and findings synthesis stay on fable. The tier rule bites on un-routed choices: never verify opus work with sonnet.
